package GUI.Ventanas_interactivas.Tipos_de_Vivienda;
...
public class TiposViviendaEliminarWindow extends JFrame {
    private JTable dataTable;
    private DefaultTableModel tableModel;
    private HashSet<Integer> deletedRowsIds; // Almacena los IDs de los registros eliminados

    public TiposViviendaEliminarWindow() {
        // Configuración de la ventana
        setTitle("Eliminar Tipos de Vivienda");
        setSize(600, 400);
        setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
        setLocationRelativeTo(null);
        setLayout(new BorderLayout());

        deletedRowsIds = new HashSet<>(); // Inicializar el conjunto de IDs eliminados

        // Mensaje superior
        JLabel messageLabel = new JLabel("Eliminar Tipos de Vivienda", SwingConstants.CENTER);
        ...
        dataTable.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() == 2) {
                    ...
                    if (row != -1) {
                        // Almacenar el ID de la fila eliminada
        ...
        // Mostrar los datos de la tabla "TiposVivienda"
        mostrarDatosTabla("TiposVivienda");
    }
    ...
    private void saveAndClose() {
        ...
        try (Connection conn = DriverManager.getConnection(url)) {
            ...
            try (PreparedStatement pstmt = conn.prepareStatement("DELETE FROM TiposVivienda WHERE id = ?")) {
